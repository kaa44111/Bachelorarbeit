...
def focal_dice_loss(pred_seg, pred_cls, target_seg, target_cls, gamma=2, alpha=0.8):
    # Segmentation loss
    bce = F.binary_cross_entropy_with_logits(pred_seg, target_seg, reduction='none')
    pt = torch.exp(-bce)
    focal_loss = alpha * (1-pt)**gamma * bce.mean()
    
    pred_sigmoid = torch.sigmoid(pred_seg)
    intersection = (pred_sigmoid * target_seg).sum()
    dice_loss = 1 - (2.*intersection + 1)/(pred_sigmoid.sum() + target_seg.sum() + 1)
    
    # Classification loss
    cls_loss = F.binary_cross_entropy(pred_cls, target_cls.float())
    
    return 0.7*(focal_loss + dice_loss) + 0.3*cls_loss

def train_model(model, dataloaders, optimizer, scheduler, num_epochs=25, save_name='model'):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, masks, labels in dataloaders[phase]:
                ...
                with autocast():
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        loss = focal_dice_loss(seg_pred, cls_pred, masks, labels)
                ...
                f1 = calculate_f1(seg_pred, masks)
            ...
            epoch_iou = total_iou / len(dataloaders[phase].dataset)
            ...
            if phase == 'val' and epoch_iou > best_iou:
                best_iou = epoch_iou
                torch.save(model.state_dict(), f'best_{save_name}.pth')
